...
public class DataDisplay extends JFrame implements Observer {
    ...
    public void generationData(int generationNumber, World world, SatEvaluator satEvaluator, SatInstance satInstance) {
        ...
        GraphicsUtil.drawString(g, "Generation " + generationNumber + "/" + IntParameter.NUM_GENERATIONS.getValue(), 5, 20, font, Color.WHITE);
        double bestFitness = satEvaluator.evaluate(satInstance, bestIndividual);
        GraphicsUtil.drawString(g, "Best fitness: " + bestFitness, 5, 35, font, Color.WHITE);
        int numIndividuals = getIndividualCount(world);
        GraphicsUtil.drawString(g, "Number of individuals: " + numIndividuals, 5, 50, font, Color.WHITE);
        int inhabitedCells = getInhabitedCellCount(world);
        GraphicsUtil.drawString(g, "Number of inhabited cells: " + inhabitedCells, 5, 65, font, Color.WHITE);
